...
<?php
    	...
    	if(isset($_POST['done'])) 
        {
			...
			$stmt->execute(array(':stat'=>'3'));
		}
 ?>
<html>
     <head>
         <title>E-CANTEEN|admcmppendg</title>
    ...
    <table class="center" style="width:75%">
      ...
      <?php
						...
						try{
							...
							{
								$sql="select * from complaint where status=1 order by date desc";
								$stmt=$pdo->query($sql);
								while($row=$stmt->fetch(PDO::FETCH_ASSOC))
								{
									...
									?>
										...
										<tr>
											...
											<td> <?=$row['compdesc']?> </td>
											<td><form method="post" action="admcmppedng.php">
												<input type="hidden"   value="<?=$row['complaintid']?>" name="userid"/>
												<input type="submit"  class="accept" value="done" name="done"/> 
												 </form>
											</td>
										</tr>
							<?php }
